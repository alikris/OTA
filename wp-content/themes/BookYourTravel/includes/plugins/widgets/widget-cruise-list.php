...
add_action( 'widgets_init', 'byt_cruise_lists_widgets' );

// Register widget.
function byt_cruise_lists_widgets() {
	global $byt_theme_globals;
	if ($byt_theme_globals->enable_cruises()) {
		register_widget( 'byt_Cruise_List_Widget' );
	}
}

// Widget class.
class byt_Cruise_List_Widget extends WP_Widget {

	/*-----------------------------------------------------------------------------------*/
	/*	Widget Setup
	/*-----------------------------------------------------------------------------------*/
	
	function byt_Cruise_List_Widget() {
	
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'byt_cruise_lists_widget', 'description' => __('BookYourTravel: Cruise List', 'bookyourtravel') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 260, 'height' => 400, 'id_base' => 'byt_cruise_lists_widget' );

		/* Create the widget. */
		$this->WP_Widget( 'byt_cruise_lists_widget', __('BookYourTravel: Cruise List', 'bookyourtravel'), $widget_ops, $control_ops );
	}
...
/*-----------------------------------------------------------------------------------*/
	...
	function widget( $args, $instance ) {
		...
		global $sc_theme_globals, $byt_cruises_post_type;
		...
		$title = apply_filters('widget_title', isset($instance['title']) ? $instance['title'] : __('Explore our latest cruises', 'bookyourtravel') );
		...
		$cruise_results = $byt_cruises_post_type->list_cruises(1, $number_of_posts, $sort_by, $order, 0, $cruise_type_ids, $cruise_tag_ids, array(), $show_featured_only);
			...
			?><div class="inner-wrap"><?php
				...
				foreach ($cruise_results['results'] as $cruise_result) {
					...
					if (isset($card_layout_classes[$posts_per_row - 1]))
						$cruise_class = $card_layout_classes[$posts_per_row - 1];
					else
...
/*-----------------------------------------------------------------------------------*/
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title' => __('Explore our latest cruises', 'bookyourtravel'),
			'number_of_posts' => '4',
			'sort_by' => 'title',
			'sort_descending' => '1',
			'display_mode' => 'card',
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number_of_posts' ) ); ?>"><?php _e('How many cruises do you want to display?', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'sort_by' ) ); ?>"><?php _e('What do you want to sort the cruises by?', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'sort_descending' ) ); ?>"><?php _e('Sort cruises in descending order?', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_featured_only') ); ?>"><?php _e('Show only featured cruises?', 'bookyourtravel') ?></label>
		...
		<p>
			<label><?php _e('Cruise type (leave blank to ignore)', 'bookyourtravel') ?></label>
			<div>
		...
		<p>
			<label><?php _e('Cruise tag (leave blank to ignore)', 'bookyourtravel') ?></label>
			<div>
		...
		<p class="cards" <?php echo ( $instance['display_mode'] != 'card' ? 'style="display:none"' : '' ); ?>>
			<label for="<?php echo esc_attr ( $this->get_field_id( 'posts_per_row' ) ); ?>"><?php _e('How many cruises do you want to display per row?', 'bookyourtravel') ?></label>
