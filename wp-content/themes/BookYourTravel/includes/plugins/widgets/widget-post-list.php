...
add_action( 'widgets_init', 'byt_post_lists_widgets' );

// Register widget.
function byt_post_lists_widgets() {
	register_widget( 'byt_Post_List_Widget' );
}

// Widget class.
class byt_Post_List_Widget extends WP_Widget {

	/*-----------------------------------------------------------------------------------*/
	/*	Widget Setup
	/*-----------------------------------------------------------------------------------*/
	
	function byt_Post_List_Widget() {
	
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'byt_post_lists_widget', 'description' => __('BookYourTravel: Post List', 'bookyourtravel') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 260, 'height' => 400, 'id_base' => 'byt_post_lists_widget' );

		/* Create the widget. */
		$this->WP_Widget( 'byt_post_lists_widget', __('BookYourTravel: Post List', 'bookyourtravel'), $widget_ops, $control_ops );
	}
...
/*-----------------------------------------------------------------------------------*/
	...
	function widget( $args, $instance ) {
		...
		global $sc_theme_globals, $byt_posts_post_type;
		...
		$title = apply_filters('widget_title', isset($instance['title']) ? $instance['title'] : __('Explore our latest offers', 'bookyourtravel') );
		...
		?><!--deals--><div class="offers clearfix full"><?php
		} else {
		...
		$post_results = $byt_posts_post_type->list_posts(0, $number_of_posts, $sort_by, $order, $category_ids);
...
/*-----------------------------------------------------------------------------------*/
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title' => __('Explore our latest offers', 'bookyourtravel'),
			'number_of_posts' => '4',
			'sort_by' => 'title',
			'sort_descending' => '1',
			'display_mode' => 'card',
		...
		<p>
			<label><?php _e('Categories', 'bookyourtravel') ?></label>
			<div>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number_of_posts' ) ); ?>"><?php _e('How many posts do you want to display?', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'sort_by' ) ); ?>"><?php _e('What do you want to sort the posts by?', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'sort_descending' ) ); ?>"><?php _e('Sort posts in descending order?', 'bookyourtravel') ?></label>
		...
		<p class="cards" <?php echo ( $instance['display_mode'] != 'card' ? 'style="display:none"' : '' ); ?>>
			<label for="<?php echo esc_attr ( $this->get_field_id( 'posts_per_row' ) ); ?>"><?php _e('How many posts do you want to display per row?', 'bookyourtravel') ?></label>
