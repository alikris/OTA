...
class byt_Search_Widget extends WP_Widget {
	...
	private $make_self_catered_searchable; 
	private $make_hotels_searchable;
...
/*-----------------------------------------------------------------------------------*/
	...
	function widget( $args, $instance ) {
		...
		$this->make_hotels_searchable = isset($instance['make_hotels_searchable'] ) ? $instance['make_hotels_searchable'] : true;
		$this->make_self_catered_searchable = isset($instance['make_self_catered_searchable'] ) ? $instance['make_self_catered_searchable'] : true;
		$this->make_tours_searchable = isset($instance['make_tours_searchable'] ) ? $instance['make_tours_searchable'] : true;
		
		if ($this->enable_accommodations && $this->make_hotels_searchable)
			$searchable_count++;
		if ($this->enable_accommodations && $this->make_self_catered_searchable)
			$searchable_count++;
		...
		$where_text = isset($instance['where_text']) ? $instance['where_text'] : __('Where?', 'bookyourtravel');
		...
		$accommodation_type_label_text = isset($instance['accommodation_type_label_text']) ? $instance['accommodation_type_label_text'] : __('Accommodation type', 'bookyourtravel');
		...
		$tour_type_label_text = isset($instance['tour_type_label_text']) ? $instance['tour_type_label_text'] : __('Tour type', 'bookyourtravel');		
		...
		$cruise_type_label_text = isset($instance['cruise_type_label_text']) ? $instance['cruise_type_label_text'] : __('Cruise type', 'bookyourtravel');	
		...
		$location_select_label_text = isset($instance['location_select_label_text']) ? $instance['location_select_label_text'] : __('Location', 'bookyourtravel');
			...
			<article class="refine-search-results byt_search_widget">
				<form class="widget-search" method="get" action="<?php echo esc_url($this->custom_search_results_page); ?>">
					...
					if ($show_what_filter)
						...
						} elseif ($this->enable_accommodations && $this->make_self_catered_searchable) {
	...
	function render_where_section($where_text, $location_select_label_text) {
		...
		$l = isset($_GET['l']) ? intval(wp_kses($_GET['l'], '')) : 0;
		...
		$location_results = $byt_locations_post_type->list_locations(0, 1, -1, 'title', 'asc');
	...
	function render_when_section($when_text, $accommodation_date_from_label_text, $accommodation_date_to_label_text, $car_rental_date_from_label_text, $car_rental_date_to_label_text, $tour_date_from_label_text, $cruise_date_from_label_text) { 
		...
		$from = isset($_GET['from']) && !empty($_GET['from'])  ? date($date_format, strtotime(wp_kses($_GET['from'], ''))) : null;
		$to = isset($_GET['to']) && !empty($_GET['to']) ? date($date_format, strtotime(wp_kses($_GET['to'], ''))) : null;
	?>
		<div class="column">
			<div class="when dt">
				<?php echo $when_text; ?>
			...
			<div class="when dd">
				<script>
					...
					window.datePickerFromValue = <?php echo $from != null ? "'" . $from . "'" : "''"; ?>;
					window.datePickerToValue = <?php echo $to != null ? "'" . $to . "'" : "''"; ?>;
				</script>
	...
	function render_cruise_type_section($cruise_type_label_text) {
		...
		if (isset($_GET['cruise_types'])) {
			$request_type_ids = BYT_Theme_Utils::retrieve_array_of_values_from_query_string('cruise_types', true);
		}
	...
	function render_accommodation_type_section($accommodation_type_label_text) {
		...
		if (isset($_GET['accommodation_types'])) {
			$request_type_ids = BYT_Theme_Utils::retrieve_array_of_values_from_query_string('accommodation_types', true);
		}
	...
	function render_car_type_section($car_type_label_text) {
		...
		if (isset($_GET['car_types'])) {
			$request_type_ids = BYT_Theme_Utils::retrieve_array_of_values_from_query_string('car_types', true);
		}
	...
	function render_tour_type_section($tour_type_label_text) {
		...
		if (isset($_GET['tour_types'])) {
			$request_type_ids = BYT_Theme_Utils::retrieve_array_of_values_from_query_string('tour_types', true);
		}
	...
	function is_hotel_what_active($what) {
		return ($this->enable_accommodations && $this->make_hotels_searchable) || $what == 1;
	}

	function is_self_catered_what_active($what) {
		return ((!$this->is_hotel_what_active($what) && $this->enable_accommodations && $this->make_self_catered_searchable) || $what == 2);
	}
	
	function is_car_rental_what_active($what) {
		return ((!$this->is_hotel_what_active(3) && 
				!$this->is_self_catered_what_active(3) && 
	...
	function is_tour_what_active($what) {
		return ((!$this->is_hotel_what_active(4) && 
				!$this->is_self_catered_what_active(4) && 
	...
	function is_cruise_what_active($what) {
		return ((!$this->is_hotel_what_active(5) && 
				!$this->is_self_catered_what_active(5) && 
				!$this->is_car_rental_what_active(5) && 
				!$this->is_tour_what_active(5) && 
				$this->enable_cruises && 
...
/*-----------------------------------------------------------------------------------*/
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['make_hotels_searchable'] = isset($new_instance['make_hotels_searchable']) ? strip_tags( $new_instance['make_hotels_searchable'] ) : '';	
		$instance['make_self_catered_searchable'] = isset($new_instance['make_self_catered_searchable']) ? strip_tags( $new_instance['make_self_catered_searchable'] ) : '';	
...
/*-----------------------------------------------------------------------------------*/
	...
	function form( $instance ) {
		...
		$defaults = array(
			...
			'enabled_search_fields_text' => __('What search fields do you want enabled?', 'bookyourtravel'),
			'what_text' => __('What?', 'bookyourtravel'),
			'when_text' => __('When?', 'bookyourtravel'),
			'where_text' => __('Where?', 'bookyourtravel'),
			...
			'accommodation_type_label_text' => __('Accommodation type', 'bookyourtravel'),
			...
			'tour_type_label_text' => __('Tour type', 'bookyourtravel'),	
			...
			'cruise_type_label_text' => __('Cruise type', 'bookyourtravel'),	
			...
			'location_select_label_text' => __('Search location', 'bookyourtravel'),
			'make_cruises_searchable' => true,
			'make_car_rentals_searchable' => true,
			'make_hotels_searchable' => true,
			'make_self_catered_searchable' => true,
		...
		<p>
			<?php _e('What general search filters do you want enabled?', 'bookyourtravel') ?>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_what_filter' ) ); ?>"><?php _e( 'Show "What" filter?', 'bookyourtravel'); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_where_filter' ) ); ?>"><?php _e( 'Show "Where" filter?', 'bookyourtravel'); ?></label>
		</p>
		
		<p style="<?php echo $byt_theme_globals->search_only_available_properties() ? 'display:block;' : 'display:none'; ?>">
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_when_filter' ) ); ?>"><?php _e( 'Show "When" filter?', 'bookyourtravel'); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_price_filter' ) ); ?>"><?php _e( 'Show "Price" filter?', 'bookyourtravel'); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_rating_filter' ) ); ?>"><?php _e( 'Show "Rating" filter?', 'bookyourtravel'); ?></label>
		</p>
		<?php } ?>	


		<h4><?php _e('General field labels', 'bookyourtravel') ?></h4>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'what_text' ) ); ?>"><?php _e('What label:', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'when_text' ) ); ?>"><?php _e('When label:', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'where_text' ) ); ?>"><?php _e('Where label:', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'location_select_label_text' ) ); ?>"><?php _e('Location label:', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'price_per_person_label_text' ) ); ?>"><?php _e("Price per person label", 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'price_per_night_label_text' ) ); ?>"><?php _e("Price per night label", 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'price_per_day_label_text' ) ); ?>"><?php _e("Price per day label", 'bookyourtravel') ?></label>
		...
		<?php if ($this->enable_reviews) { ?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'user_rating_label_text' ) ); ?>"><?php _e("User rating label", 'bookyourtravel') ?></label>
		...
		<?php $accommodation_fields_visible = ($instance['make_hotels_searchable'] || $instance['make_self_catered_searchable']); ?>
		<?php $hotel_fields_visible = ($instance['make_hotels_searchable']); ?>

		<p>
			<?php _e('For accommodations', 'bookyourtravel') ?>
		</p>
		
		<p>
			<input id="<?php echo esc_attr( $this->get_field_id( 'make_hotels_searchable' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'make_hotels_searchable' ) ); ?>" type="checkbox" value="1" <?php checked( $instance['make_hotels_searchable'] ); ?> />
			<label for="<?php echo esc_attr( $this->get_field_id( 'make_hotels_searchable' ) ); ?>"><?php _e( 'Make hotels searchable?', 'bookyourtravel'); ?></label>
		</p>
		
		<p>
			<input id="<?php echo esc_attr( $this->get_field_id( 'make_self_catered_searchable' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'make_self_catered_searchable' ) ); ?>" type="checkbox" value="1" <?php checked( $instance['make_self_catered_searchable'] ); ?> />
			<label for="<?php echo esc_attr( $this->get_field_id( 'make_self_catered_searchable' ) ); ?>"><?php _e( 'Make self-catered searchable?', 'bookyourtravel'); ?></label>
		</p>
		...
		<p style="<?php echo $accommodation_fields_visible ? '' : 'display:none'; ?>">
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_accommodation_type_filter' ) ); ?>"><?php _e( 'Show "Accommodation type" filter?', 'bookyourtravel'); ?></label>
		</p>
		...
		<p style="<?php echo $accommodation_fields_visible ? '' : 'display:none'; ?>">
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_stars_filter' ) ); ?>"><?php _e( 'Show "Stars" filter?', 'bookyourtravel'); ?></label>
		</p>
		
		<p style="<?php echo $hotel_fields_visible ? '' : 'display:none'; ?>">
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_room_count_filter' ) ); ?>"><?php _e( 'Show "Rooms" filter?', 'bookyourtravel'); ?></label>
		</p>		
		
		<p style="<?php echo $accommodation_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'accommodation_date_from_label_text' ) ); ?>"><?php _e('Accommodation date from label:', 'bookyourtravel') ?></label>
		...
		<p style="<?php echo $accommodation_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'accommodation_date_to_label_text' ) ); ?>"><?php _e('Accommodation date to label:', 'bookyourtravel') ?></label>
		...
		<p style="<?php echo $accommodation_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'accommodation_location_label_text' ) ); ?>"><?php _e('Accommodation location label:', 'bookyourtravel') ?></label>
		...
		<p style="<?php echo $hotel_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'rooms_label_text' ) ); ?>"><?php _e('Rooms label:', 'bookyourtravel') ?></label>
		...
		<p style="<?php echo $accommodation_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'accommodation_type_label_text' ) ); ?>"><?php _e("Accommodation type label", 'bookyourtravel') ?></label>
		...
		<p style="<?php echo $accommodation_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'star_rating_label_text' ) ); ?>"><?php _e("Star rating label", 'bookyourtravel') ?></label>
		...
		<p>
			<?php _e('For tours', 'bookyourtravel') ?>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'make_tours_searchable' ) ); ?>"><?php _e( 'Make tours searchable?', 'bookyourtravel'); ?></label>
		</p>
		...
		<p style="<?php echo $tour_fields_visible ? '' : 'display:none'; ?>">
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_tour_type_filter' ) ); ?>"><?php _e( 'Show "Tour type" filter?', 'bookyourtravel'); ?></label>
		</p>
		
		<p style="<?php echo $tour_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'tour_date_from_label_text' ) ); ?>"><?php _e('Tour date from label:', 'bookyourtravel') ?></label>
		...
		<p style="<?php echo $tour_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'tour_type_label_text' ) ); ?>"><?php _e("Tour type label", 'bookyourtravel') ?></label>
		...
		<p style="<?php echo $tour_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'tour_location_label_text' ) ); ?>"><?php _e('Tour location label:', 'bookyourtravel') ?></label>
		...
		<p>
			<?php _e('For car rentals', 'bookyourtravel') ?>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'make_car_rentals_searchable' ) ); ?>"><?php _e( 'Make car rentals searchable?', 'bookyourtravel'); ?></label>
		</p>
		...
		<p style="<?php echo $car_rental_fields_visible ? '' : 'display:none'; ?>">
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_car_type_filter' ) ); ?>"><?php _e( 'Show "Car type" filter?', 'bookyourtravel'); ?></label>
		</p>
		
		<p style="<?php echo $car_rental_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'car_rental_date_from_label_text' ) ); ?>"><?php _e('Car rental date from label:', 'bookyourtravel') ?></label>
		...
		<p style="<?php echo $car_rental_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'car_rental_date_to_label_text' ) ); ?>"><?php _e('Car rental date to label:', 'bookyourtravel') ?></label>
		...
		<p style="<?php echo $car_rental_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'car_rental_location_label_text' ) ); ?>"><?php _e('Car rental location label:', 'bookyourtravel') ?></label>
		...
		<p style="<?php echo $car_rental_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'car_type_label_text' ) ); ?>"><?php _e("Car type label", 'bookyourtravel') ?></label>
		...
		<p>
			<?php _e('For cruises', 'bookyourtravel') ?>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'make_cruises_searchable' ) ); ?>"><?php _e( 'Make cruises searchable?', 'bookyourtravel'); ?></label>
		</p>
		...
		<p style="<?php echo $cruise_fields_visible ? '' : 'display:none'; ?>">
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_cruise_type_filter' ) ); ?>"><?php _e( 'Show "Cruise type" filter?', 'bookyourtravel'); ?></label>
		</p>
		...
		<p style="<?php echo $cruise_fields_visible ? '' : 'display:none'; ?>">
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_cabin_count_filter' ) ); ?>"><?php _e( 'Show "Cabin count" filter?', 'bookyourtravel'); ?></label>
		</p>
		
		<p style="<?php echo $cruise_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'cruise_date_from_label_text' ) ); ?>"><?php _e('Cruise date from label:', 'bookyourtravel') ?></label>
		...
		<p style="<?php echo $cruise_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'cabins_label_text' ) ); ?>"><?php _e('Cabins label:', 'bookyourtravel') ?></label>
		...
		<p style="<?php echo $cruise_fields_visible ? '' : 'display:none'; ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'cruise_type_label_text' ) ); ?>"><?php _e('Cruise type label:', 'bookyourtravel') ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'submit_button_text' ) ); ?>"><?php _e('Search submit button text', 'bookyourtravel') ?></label>
